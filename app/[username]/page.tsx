...
export default function PublicCardPage({ params }: { params: { username: string } }) {
  ...
  useEffect(() => {
    const found = getProfile(params.username);
    setProfile(found && found.isPublic ? found : null);
  }, [params.username]);
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-paper px-6 text-center">
        ...
        <p className="max-w-sm text-sm text-ink-soft">
          LinkCard cards in this demo live in your browser&apos;s storage, so a card only shows up
          on the device that published it. Create your own to see it live.
        </p>
        <Link
          href="/create"
